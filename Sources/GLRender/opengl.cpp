...
MeshBuffer Renderer::createMeshBuffer(const std::shared_ptr<class ::MeshData>& mesh, uint32_t buf_flag)
{
	...
	//glNamedBufferStorage(buffer.vbo, vertexDataSize, reinterpret_cast<const void*>(&mesh->vertices()[0]), buf_flag);
	...
	//glNamedBufferStorage(buffer.ibo, indexDataSize, reinterpret_cast<const void*>(&mesh->faces()[0]), buf_flag);
